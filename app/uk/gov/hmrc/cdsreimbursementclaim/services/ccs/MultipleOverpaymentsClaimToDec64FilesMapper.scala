...
import uk.gov.hmrc.cdsreimbursementclaim.models.claim.{ClaimSubmitResponse, MultipleOverpaymentsClaimRequest}
import uk.gov.hmrc.cdsreimbursementclaim.models.dates.TemporalAccessorOps

import java.util.UUID

class MultipleOverpaymentsClaimToDec64FilesMapper extends ClaimToDec64Mapper[MultipleOverpaymentsClaimRequest] {

  def map(request: MultipleOverpaymentsClaimRequest, response: ClaimSubmitResponse): List[String] =
    request.claim.supportingEvidences.zipWithIndex.map { case (document, index) =>
      DEC64RequestBuilder(
        ...
        properties = List(
          ...
          ("DeclarationId", request.claim.leadMrn.value),
          ("DeclarationType", "MRN"),
