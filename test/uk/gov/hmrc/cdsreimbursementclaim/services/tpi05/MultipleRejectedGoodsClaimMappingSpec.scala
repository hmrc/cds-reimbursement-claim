  ...
  "The Reject Goods claim mapper" should {
    ...
    "map a valid Multiple claim to TPI05 request" in forAll {
      details: (MultipleRejectedGoodsClaim, List[DisplayDeclaration]) =>
        ...
        val tpi05Request          = multipleRejectedGoodsClaimToTPI05Mapper.map((claim, declarations))
          ...
          details should have(
            ...
            'EORIDetails (
              EoriDetails(
                ...
                importerEORIDetails = {
                  val maybeConsigneeDetails = leadDeclaration.displayResponseDetail.consigneeDetails
    ...
    "fail with the error" when {
      ...
      "cannot find NDRC details for claimed reimbursement" in forAll {
        (details: (MultipleRejectedGoodsClaim, List[DisplayDeclaration]), taxCode: TaxCode) =>
          ...
          val tpi05Request = multipleRejectedGoodsClaimToTPI05Mapper.map((updatedClaim, updatedDeclarations))
