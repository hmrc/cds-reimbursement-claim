...
import uk.gov.hmrc.cdsreimbursementclaim.models.ids.MRN._
...
class MRNSpec extends AnyWordSpec with Matchers {
  ...
  "MRN validation" should {
    ...
    "Succeed on parsing" in {
      val mrnString = "57WAFResponse11111"
      parse(mrnString) shouldBe Some(MRN(mrnString))
    }

    "Has PathBindable" in {
      val mrnString = "57WAFResponse11111"
      implicitly[PathBindable[MRN]].bind("", mrnString) shouldBe Right(MRN(mrnString))
    }
