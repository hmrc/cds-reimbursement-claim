...
class ClaimTransformerSpec extends AnyWordSpec with Matchers with MockFactory {
  ...
  case class SampleClass(id: Int, name: String, CDFPayCaseNumber: String, declarationID: Option[String]) extends ClaimItem

  "ClaimTransformer" should {
    "removeDuplicates" in {
      val list = Seq(SampleClass(1, "Bob", "", None), SampleClass(1, "Bob", "", None), SampleClass(2, "Charlie", "", None))
